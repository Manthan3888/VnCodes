...
function mapApiTemplate(t: any): Template {
  return {
    id: t.id,
    title: t.title,
...
export default function TemplateDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([templatesApi.getTemplateById(id), templatesApi.getTemplates()])
      .then(([detailRes, listRes]) => {
        if (detailRes.template) {
          setTemplate(mapApiTemplate(detailRes.template));
          const list = (listRes.templates || []).map(mapApiTemplate);
  ...
  if (!template) {
    return (
      <main className="mx-auto w-full max-w-6xl bg-white px-4 py-10 dark:bg-black sm:px-6">
        <div className="flex items-center justify-center py-12">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">Template not found.</p>
        </div>
      </main>
    );
  }

  const formattedDate = template.date
